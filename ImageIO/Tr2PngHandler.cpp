...
bool g_pngComplainNoCutout = false;
TRI_REGISTER_SETTING( "pngComplainNoCutout", g_pngComplainNoCutout );
...
bool Tr2PngHandler::SetStream( IBlueStream* src )
{
	if( !Tr2ImageHandler::SetStream( src ))
	{
		return false;
	}
	
	png_byte pngsig[ 8 ];
	if( !m_source || m_source->Read( pngsig, sizeof( pngsig ) ) == -1 )
	{
		return false;
	}
	...
	png_set_read_fn ( m_png, this, &Tr2PngHandler::ReadData_thunk );
	png_set_error_fn( m_png, this, &Tr2PngHandler::UserError_thunk, &Tr2PngHandler::UserWarning_thunk );
	
	return true;
}

void Tr2PngHandler::ReadData( png_bytep data, png_size_t length )
{
	CCP_STATS_ZONE( __FUNCTION__ );

	if( m_source )
	{
		m_haveError = ( m_source->Read( data, length ) == -1 );
	}
}
...
void Tr2PngHandler::ReadData_thunk(png_structp pngPtr, png_bytep data, png_size_t length)
{
	...
	static_cast<Tr2PngHandler*>( png_get_io_ptr( pngPtr ) )->ReadData( data, length );
}
...
void Tr2PngHandler::UserError_thunk( png_structp pngPtr, png_const_charp errorMsg)
{
	...
	static_cast<Tr2PngHandler*>( png_get_io_ptr( pngPtr ) )->UserError( errorMsg );
}
...
void Tr2PngHandler::UserWarning_thunk( png_structp pngPtr, png_const_charp warningMsg)
{
	...
	static_cast<Tr2PngHandler*>( png_get_io_ptr( pngPtr ) )->UserWarning( warningMsg );
}
...
bool Tr2PngHandler::ReadHeader()
{
	...
	if( g_pngComplainNoCutout && !isPalette )
	{
		if( m_cutoutX      == 0.0f	&& 
			m_cutoutY      == 0.0f	&&
			m_cutoutWidth  == 1.0f	&&
			m_cutoutHeight == 1.0f	&& 
			m_width  > 32			&&	// don't complain if not worth the hassle
			m_height > 32			&&
			m_bitsPerPixel >= 24 )		// don't complain if there's a good reason to use PNG (8/16bit kung fu)
		{
			CCP_LOGWARN( "No cutout at all for %S - consider using DDS - %d x %d, %d bpp", m_sourceName.c_str(), m_width, m_height, m_bitsPerPixel );
		}
	}
...
bool Tr2PngHandler::IsSupported()
{
	if(	m_bitsPerPixel == 8  ||
		m_bitsPerPixel == 16 ||
		m_bitsPerPixel == 24 ||
...
bool Tr2PngHandler::ReadImage()
{
	...
	{
		CCP_STATS_ZONE( __FUNCTION__ " swap" );
...
{
	IBlueStream* output = reinterpret_cast<IBlueStream*>( png_get_io_ptr( png ) );
...
//   false Otherwise
// --------------------------------------------------------------------------------------
bool Tr2PngHandler::Save( const Tr2HostBitmap& image, IBlueStream* output )
{
	if( !image.IsValid() )
	{
	...
	if( colorType == PNG_COLOR_TYPE_RGB )
	{
		for( unsigned i = 0; i < image.GetHeight(); ++i )
		{
			...
			const char* srcRow = image.GetRawData() + i * image.GetPitch();
			char* destRow = ( char* )rows[i];
			...
			if( is1010102 )
			{
				for( unsigned j = 0; j != image.GetWidth(); ++j, destRow += 3, srcRow += 4 )
				{
					...
					destRow[bSwizzle] = ( ( rgb10 >>  0 ) & 1023 ) >> 2;
					destRow[1] = ( ( rgb10 >> 10 ) & 1023 ) >> 2;
					destRow[rSwizzle] = ( ( rgb10 >> 20 ) & 1023 ) >> 2;
				}
			}
			else
			{
